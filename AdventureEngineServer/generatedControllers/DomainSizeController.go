...
func GetDomainSizes(context *contextProviders.GeneratedControllerContext[types.DomainSize, dtos.DomainSizeDTO, contextProviders.GetArgs[types.DomainSize], contextProviders.GetReturn[types.DomainSize]]) {
   if context == nil {
   ...
   serviceBuffer, err := context.ServiceAction(&contextProviders.GetArgs[types.DomainSize]{ Filters: &parsedFilters })
   if err != nil {
   ...
   for _, dbTypeInstance := range serviceBuffer {
      ...
      if (pointerToDTO == nil) {
         context.RequestContext.IndentedJSON(http.StatusInternalServerError, errors.New("DTO conversion resulted in nil for object of type DomainSize"))
         return
      }
...
func GetDomainSizeById(context *contextProviders.GeneratedControllerContext[types.DomainSize, dtos.DomainSizeDTO, contextProviders.GetByIdArgs[types.DomainSize], contextProviders.GetByIdReturn[types.DomainSize]]) {
   if context == nil {
      panic("No controller context provided")
   }
   ...
   serviceBuffer, err := context.ServiceAction(&contextProviders.GetByIdArgs[types.DomainSize]{ Id: idNum })
   if err != nil {
      context.RequestContext.IndentedJSON(http.StatusInternalServerError, err.Error())
      return
   }
...
func SaveDomainSize(context *contextProviders.GeneratedControllerContext[types.DomainSize, dtos.DomainSizeDTO, contextProviders.SaveArgs[types.DomainSize], contextProviders.SaveReturn[types.DomainSize]]) {
   if context == nil {
      panic("No controller context provided")
   }
   ...
   var DTOBuffer *dtos.DomainSizeDTO = &dtos.DomainSizeDTO{}
   var batchDTOBuffer []*dtos.DomainSizeDTO
   ...
   if err := context.RequestContext.ShouldBindBodyWith(DTOBuffer, binding.JSON); err == nil {
      ...
      serviceBuffer = []*types.DomainSize{context.DTOFlattener(DTOBuffer)}
      serviceReturn, err := context.ServiceAction(&contextProviders.SaveArgs[types.DomainSize]{ Items: serviceBuffer })
      if err != nil {
         context.RequestContext.IndentedJSON(http.StatusInternalServerError, err.Error())
         return
      }
      ...
      serviceBuffer = utils.Map(batchDTOBuffer, func(dto *dtos.DomainSizeDTO) *types.DomainSize { return context.DTOFlattener(dto) })
      serviceReturn, err := context.ServiceAction(&contextProviders.SaveArgs[types.DomainSize]{ Items: serviceBuffer })
      if err != nil {
         context.RequestContext.IndentedJSON(http.StatusInternalServerError, err.Error())
         return
      ...
      returnBuffer, err := utils.ErrorCompatibleMap(serviceReturn, func(dbReturn *types.DomainSize) (*dtos.DomainSizeDTO, error) { return context.DTOConverter(dbReturn) })
