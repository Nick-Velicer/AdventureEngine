...
def produceTSServiceFile(typeName: str):
    ...
    def produceGetCollectionMethod(tableName: str):
        lines = [
            'export async function get' + typeName + 's(filter?: FilterAST<' + typeName + '>): Promise<' + typeName + '[]> {',
            *indentLineBlock([
                'try {',
                *indentLineBlock([
                    'const response = await fetch("' + baseApiUrl + 'get' + tableName + 's" + produceFilterExtensionFromAST(filter));',
