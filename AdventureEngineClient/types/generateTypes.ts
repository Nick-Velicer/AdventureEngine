...
const internalConstantStructure = (typeNames: Array<string>) => {
	return ["const BaseAppTypes = {"].concat(typeNames.map((typeName) => '	' + typeName + ': {} as Base__' + typeName + ',')).concat(["} as const satisfies Record<string, SchemaObject> "]).join("\n");
}
...
writeFileSync(appTypesPath + "/appTypes.ts", (
	...
	"\n\n" + 
	internalConstantStructure(typeNames) + 
	"\n\n" +
